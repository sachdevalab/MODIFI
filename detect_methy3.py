...
def run_ipdtools(fasta, out, threads=1):      
    cmd = f"ipdtools -f {fasta} -o {out} -p -n {threads} --model P6-C4 --verbosity INFO"
    os.system(cmd)
...
out = 'borg/customized/ipdtools.csv'


# print ("start")
# test_modified_pos, benchmark_modified_pos = parse_gff(gff)
# print ("gff is loaded")
# IPD_ratio_list, reverse_IPD_ratio_list, summary_IPD_list, summary_reverse_IPD_list, control_list, reverse_control_list = extract_ipd_ratio(csv)
# print ("csv is loaded")
# observed_IPD_list, observed_IPD_reverse_list = read_hifi_bam(bam_file)
# observed_IPD_list, observed_IPD_reverse_list = read_subread_bam(subread_bam)
# print ("bam is loaded", np.median(observed_IPD_list), np.median(observed_IPD_reverse_list))

# observed_IPD_list, observed_IPD_reverse_list = baseline_correction(observed_IPD_list, observed_IPD_reverse_list)

run_ipdtools(fasta, out)
# my_contig = Contig()

# my_contig.play_ipd3(IPD_ratio_list, reverse_IPD_ratio_list,\
#                      control_list, reverse_control_list,\
#                     observed_IPD_list, observed_IPD_reverse_list,\
#                     summary_IPD_list, summary_reverse_IPD_list)

# os.system("Rscript plot_line2.R")
