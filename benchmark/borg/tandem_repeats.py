...
def get_id_to_header_seq(in_seq_path):
    id_to_header_seq = {}
    ...
    for header, seq in SimpleFastaParser(smart_open.open(in_seq_path)):
        ...
        description = " ".join(header_split[1:])

        id_to_header_seq[id_] = (description, seq)
        
    return id_to_header_seq
    ...
    for line in tandem_repeat_match_result_vals:
        ...
        if (
            ...
            if (
                ...
                tandem_repeat_seq_gc =gc_fraction(tandem_repeat_seq) * 100
...
def write_tandem_output(parallel_tandem_results, id_to_header_seq):
    ...
    # Remove trailing slash to get proper basename
    # out_dir_name = os.path.basename(out_dir_path.rstrip('/'))
    out_dir_name = out_dir_path
    # Fallback if still empty
    if not out_dir_name:
        out_dir_name = "tandem_repeats_output"
        ...
        for tandem_result in parallel_tandem_results:
            if tandem_result is not None:
                ...
                for tandem_info_type_to_val in tandem_infos:
                    seq_description = id_to_header_seq[id_][0]
                    tandem_info_type_to_vals["sequence_id"].append(id_)
                    # tandem_info_type_to_vals["sequence_description"].append(seq_description)
    ...
    for id_, group in df_tandem_info.groupby("sequence_id"):
        ...
        summary_info_to_vals["sequence_description"].append(seq_description)
    ...
    repeat_bt = BedTool(out_bed_path)
    gene_bt = BedTool(args.in_gff)
    
    repeat_gene_intersect_bt = repeat_bt.intersect(gene_bt, wo=True, f=1)

    df_repeat_gene = defaultdict(list)
    
    for feature in repeat_gene_intersect_bt:                        
        if feature.fields[14] == "CDS":   
            df_repeat_gene["sequence_id"].append(feature.chrom)
            df_repeat_gene["repeat_num"].append(feature.name.split("_")[1])
        
    df_repeat_gene = pd.DataFrame(df_repeat_gene)
        
    repeat_seq_repeat_ids = set()
        
    for i,row in df_repeat_gene.iterrows():
        repeat_seq_repeat_ids.add((row["sequence_id"], row["repeat_num"]))
    
    df_tandem_info["in_cds"] = df_tandem_info.apply(lambda row: "yes" if (str(row['sequence_id']), str(row['repeat_num'])) in repeat_seq_repeat_ids else "no", axis=1)
    df_tandem_info.to_csv(out_table_path, sep="\t", index=False)


def main():
    id_to_header_seq = get_id_to_header_seq(args.in_seqs)
    ...
    tandem_results = parallel_run_get_tandem_repeats(
        args.thread,
        id_seq_groups,
        "/shared/software/mummer/v3.23/bin/repeat-match",
        args.min_repeat_len,
    ...
    write_tandem_output(tandem_results, id_to_header_seq)
