...
Writes: jf8_wgadb_family_matrix.csv, jf8_wgadb_boldpos.csv (in borg/revision/motif_class)
"""
import pandas as pd
...
mat.to_csv(f"{D}/jf8_wgadb_family_matrix.csv")

cpmap = dict(ROWS)
pd.DataFrame([(r, cpmap[r], 'A') for r in order], columns=["motif","centerPos","modified_base"]
             ).to_csv(f"{D}/jf8_wgadb_boldpos.csv", index=False)
