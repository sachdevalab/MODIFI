...
def main():
    ...
    ap.add_argument("--base", default="bg_300",
                    help="base community whose manifest defines the 58 donors + 242 bg "
                         "(bg_300 for rep1; bg_300_rep2..5 for reps 2-5)")
    ap.add_argument("--tag", default="", help="replicate suffix, e.g. rep2 -> bg300_k<K>_rep2")
    ap.add_argument("--seed", type=int, default=42, help="read-subsample seed (match the rep's seed)")
    ap.add_argument("--threads", type=int, default=64)
    ap.add_argument("--keep-prepped", action="store_true")
    a = ap.parse_args()

    donors, background, mge = bc.load_pool()
    man = pd.read_csv(f"{bc.OUT_ROOT}/{a.base}/{a.base}.manifest.csv")
    ...
    label = f"bg300_k{a.K}" + (f"_{a.tag}" if a.tag else "")
    print(f"[{label}] base={a.base} donors={len(donor_rows)} ({donor_rows['Species'].nunique()} species) "
