...
melt_df <- read_csv("../../tmp/rev_figs/ece_anno/largest_component_binary_matrix_melted.csv", show_col_types = FALSE)
# Label ECE columns by their 95%-ANI cluster-representative id (consistent with the linkage/kp naming;
# e.g. the cross-genus hub plasmid shows as infant_15_35_C, not an arbitrary alphabetically-first member)
melt_df$MGE_label <- melt_df$MGE_node
...
if (length(non_virus_rows) > 1) {
  ...
  # Order rows (MGEs) by degree (number of host links), descending
  row_order <- order(rowSums(sub_nv), decreasing = TRUE)
} else {
  row_order <- seq_along(non_virus_rows)
}

# Order columns (hosts) by degree; ascending so the highest-degree host sits at the top of the y-axis
if (length(non_virus_rows) > 0 && ncol(binary_matrix) > 1) {
  col_order <- order(colSums(binary_matrix), decreasing = FALSE)
} else {
...
if (length(virus_rows) > 0) {
  virus_rows <- virus_rows[order(rowSums(binary_matrix[virus_rows, , drop = FALSE]), decreasing = TRUE)]
...
host_labels_df <- host_info %>%
  ...
  mutate(label = paste0("italic('", host_species, "')~'(", host_label, ")'"))
...
p <- ggplot(plot_data, aes(x = MGE_node, y = host_node, fill = factor(present))) +
  ...
  scale_y_discrete(labels = function(br) parse(text = ordered_col_labels[br])) +
...
# Save plot
ggsave("../../tmp/rev_figs/ece_anno/fig5d_largest_component.pdf",
       plot = combined_plot, width = 9, height = 6, dpi = 300)
ggsave("../../tmp/rev_figs/ece_anno/fig5d_largest_component.png", plot = combined_plot, width = 9, height = 6, dpi = 200)
