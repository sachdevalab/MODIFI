...
FT_OUT = Path("/home/shuaiw/borg/paper/ipdsummary/soil_1/fibertools.out")
...
def main() -> int:
    ...
    wide_path = OUT_DIR / "runtime_3tools_wide.csv"
    long_path = OUT_DIR / "runtime_3tools_long.csv"
    ...
    for ref_name in REFS:
        ...
        # fibertools: sum of ft predict-m6a + ft pileup (the calling steps, HiFi).
        ft_predict = parse_gnu_time_verbose(FT_OUT / f"{base}.ft_predict.time")
        ft_pileup = parse_gnu_time_verbose(FT_OUT / f"{base}.ft_pileup.time")
        if ft_predict and ft_pileup:
            long_rows.append(
                {
                    "reference_fa": ref_name,
                    "ref_bases": ref_bases_s,
                    "software": "fibertools",
                    "cpu_hr": f'{ft_predict["cpu_hr"] + ft_pileup["cpu_hr"]:.6f}',
                    "wall_hr": f'{ft_predict["wall_hr"] + ft_pileup["wall_hr"]:.6f}',
                    "max_rss_gb": f'{max(ft_predict["max_rss_gb"], ft_pileup["max_rss_gb"]):.6f}',
                    "time_file": str(FT_OUT / f"{base}.ft_predict.time")
                    + ";" + str(FT_OUT / f"{base}.ft_pileup.time"),
                }
            )
