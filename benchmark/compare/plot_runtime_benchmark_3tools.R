...
in_csv <- file.path(modifi_root, "tmp/figures/base_benchmark/runtime_3tools_long.csv")
out_pdf <- file.path(modifi_root, "tmp/figures/base_benchmark/runtime_vs_refsize_3tools.pdf")
out_png <- file.path(modifi_root, "tmp/figures/base_benchmark/runtime_vs_refsize_3tools.png")
...
raw <- read_csv(in_csv, show_col_types = FALSE) %>%
  mutate(
    ...
    software = factor(software, levels = c("MODIFI", "ipdSummary", "fibertools"))
  )
...
pal <- c(MODIFI = "#0f766e", ipdSummary = "#b45309", fibertools = "#7c3aed")
