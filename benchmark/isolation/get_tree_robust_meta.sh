...
input_dir="/home/shuaiw/borg/paper/isolation/GTDB_tree/meta_genomes/"
out_dir="/home/shuaiw/borg/paper/isolation/GTDB_tree/meta_gtdb_tree"
...
# if [ $? -eq 0 ]; then
#     echo "GTDB-Tk bacteria tree completed successfully!"
#     echo "Results available in: $out_dir"
# else
#     echo "Error: GTDB-Tk infer failed for bacteria"
#     exit 1
# fi


## also build a tree for archaea
echo "Running GTDB-Tk infer for archaea..."
gtdbtk infer --msa_file $out_dir/align/gtdbtk.ar53.user_msa.fasta.gz \
        --out_dir $out_dir \
        --prefix archaea \
        --cpus 64

if [ $? -eq 0 ]; then
    echo "GTDB-Tk archaea tree completed successfully!"
    echo "Archaea tree results available in: $out_dir"
else
    echo "Warning: GTDB-Tk infer failed for archaea (may not have archaea genomes)"
fi
