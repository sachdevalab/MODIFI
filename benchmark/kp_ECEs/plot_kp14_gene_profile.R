...
bgc <- read.delim(file.path(FIG,"kp14_antismash_summary.tsv"), stringsAsFactors=FALSE)
ann <- merge(ann, bgc[,c("representative","n_BGC")], by="representative", all.x=TRUE)
ann$n_BGC[is.na(ann$n_BGC)] <- 0
...
# single legend for all binary yes/no tracks (Circular, Known, AMR, Toxin, Virulence, BGC)
pres_lgd <- Legend(title="binary tracks", labels=c("yes (coloured)","no"),
                   legend_gp=gpar(fill=c("grey30","#f0f0f0")),
...
draw_hm <- function(mat_csv, outbase, title, cluster_cols, col_label_map=NULL){
  ...
  nbgc <- suppressWarnings(as.integer(ann$n_BGC)); nbgc[is.na(nbgc)] <- 0
  la <- rowAnnotation(
    Type = ann$rep_type,
    Mobility = mob,
    ...
    col = list(Type=c(plasmid="#2171b5", virus="#31a354"),
               ...
               BGC=c(yes="#d94801", no="#f0f0f0")),
    show_legend=c(Type=TRUE, Mobility=TRUE, Circular=FALSE, `Known (ANI)`=FALSE,
                  AMR=FALSE, Toxin=FALSE, Virulence=FALSE, BGC=FALSE),
    annotation_name_gp=gpar(fontsize=8.5), annotation_name_side="top",
    simple_anno_size=unit(4,"mm"), gap=unit(0.6,"mm"),
    annotation_legend_param=list(Type=legpar,Mobility=legpar))
  ...
  draw(ht, merge_legend=TRUE, heatmap_legend_side="right", annotation_legend_side="right", annotation_legend_list=list(pres_lgd),
