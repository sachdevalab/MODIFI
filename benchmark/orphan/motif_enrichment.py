...
if __name__ == "__main__":
    ...
    data = []
    i = 0
    for file in os.listdir(f"{methy_dir}/motifs/"):
        if file.endswith(".motifs.csv"):
            ## check if the file has more than one line
            with open(f"{methy_dir}/motifs/{file}", "r") as f:
                lines = f.readlines()
                if len(lines) < 2:
                    # print(f"File {file} has less than two lines, skipping.")
                    continue
            genome = file.split(".")[0]
            my_ref = f"{methy_dir}/contigs/{genome}.fa"
            gff = f"{methy_dir}/gffs/{genome}.gff"
            motif_file = f"{methy_dir}/motifs/{genome}.motifs.csv"
            REF = read_ref(my_ref)
            for ctg in REF:
                ctg_len = len(REF[ctg])
            if ctg_len < min_ctg_len:
    ...
    df["significant"] = df["corrected_p_value"] < 0.05
    ## sort the dataframe by corrected_p_value
